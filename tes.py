...
def update_dizipal_url(file_path):
...
def git_commit_and_push(repo_path, files, message):
    repo = Repo(repo_path)
...
repo_path = os.getcwd()
kotlin_file = os.path.join(repo_path, "src/main/kotlin/com/Prueba/DizipalV2.kt")
gradle_file = os.path.join(repo_path, "build.gradle.kts")

new_url = update_dizipal_url(kotlin_file)
if new_url:
    update_version(gradle_file)
    git_commit_and_push(repo_path, [kotlin_file, gradle_file], f"URL güncellendi: {new_url}")
